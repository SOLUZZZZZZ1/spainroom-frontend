// ==============================
// FILE: src/components/TablaCedulas.jsx
// ==============================
...
export default function TablaCedulas(){
  ...
  useEffect(()=>{
    const run = async ()=>{
      try{
        setError('')
        const r = await fetch('/data/mapa_cedula_provincias.csv', {cache:'no-cache'})
        ...
        const iDate = findCol(['fecha','verificacion'], 11)

        const out = []
        let bestDate = ''
        for(let i=1;i<lines.length;i++){
          const cols = lines[i].split(delim)
          const provincia = cols[iProv] || ''
          const oblRaw = String(cols[iObl]||'').trim().toLowerCase()
          const doc    = cols[iDoc] || ''
          const org    = cols[iOrg] || ''
          const vig    = cols[iVig] || ''
          const notas  = cols[iNotas] || ''
          const link   = cols[iLink] || ''
          const fecha  = cols[iDate] || ''

          let cat = 'No'
          if (/^si|sí$/i.test(oblRaw) || oblRaw.startsWith('si')) cat='Sí'
          else if (oblRaw.startsWith('dep')) cat='Depende'

          out.push({provincia, obligatorio:cat, doc, org, vig, notas, link, fecha})
          if (fecha && (!bestDate || fecha > bestDate)) bestDate = fecha
        }
        setRows(out)
        setLastDate(bestDate)
      }catch(e){
        console.error(e); setError('No se pudo cargar el CSV /data/mapa_cedula_provincias.csv')
      }
    }
    run()
  },[])
