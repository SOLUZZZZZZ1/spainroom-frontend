...
// formatea bonito si es +34 móvil (opcional)
function prettyPhone(e164) {
  const m = /^\+34(\d{9})$/.exec(e164.replace(/\s+/g,""));
  if (!m) return e164 || "—";
  const n = m[1]; // 9 dígitos
...
export default function PhoneCTA() {
  ...
  const [slot, setSlot] = useState("10:00–13:00");
...
URL: ${location.href}`
  ...
  const mailtoHref = () => {
    const subject = encodeURIComponent("Solicitud de llamada — Propietarios SpainRoom");
  ...
  async function copyPhone() {
    try { await navigator.clipboard.writeText(PHONE); alert("Número copiado"); } catch {}
  }

  return (
    <>
      {!open && (
        <button title="Atención telefónica" onClick={()=>setOpen(true)} style={fabBtn}>📞</button>
      )}
      ...
      {open && (
        <div style={panel}>
          ...
          <div style={{padding:12, display:"grid", gap:10}}>
            {/* Línea SIEMPRE visible con el número (PC y móvil) */}
            {PHONE && (
              <div style={phoneLine}>
                <span>📞</span>
            ...
            {/* Botón “Llamar ahora” SOLO en móvil (acción directa) */}
            {isMobile && PHONE && (
            ...
            {/* Te llamamos */}
            <div style={box}>
              <div style={{fontWeight:700, marginBottom:6}}>¿Te llamamos?</div>
              <input placeholder="Tu nombre" value={name} onChange={e=>setName(e.target.value)} style={inp}/>
              <input placeholder="Tu teléfono" value={num} onChange={e=>setNum(e.target.value)} style={inp}/>
